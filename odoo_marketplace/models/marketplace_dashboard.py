...
class marketplace_dashboard(models.Model):
            ...
            if rec.state == 'product':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('project', '=', False), ('marketplace_seller_id.user_ids', '=', self._uid), ('status', '=', 'approved')])
                else:
                    obj = self.env['product.template'].search(
                        [('project', '=', False), ('marketplace_seller_id', '!=', False), ('status', '=', 'approved')])
                rec.count_product_approved = len(obj)
            elif rec.state == 'project':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('project', '=', True), ('marketplace_seller_id.user_ids', '=', self._uid), ('status', '=', 'approved')])
                else:
                    obj = self.env['product.template'].search(
                        [('project', '=', True), ('marketplace_seller_id', '!=', False), ('status', '=', 'approved')])
                rec.count_product_approved = len(obj)
    ...
    def _get_pending_count(self):
        for rec in self:
            if rec.state == 'product':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('project', '=', False), ('marketplace_seller_id.user_ids', '=', rec._uid), ('status', '=', 'pending')])
                else:
                    obj = self.env['product.template'].search(
                        [('project', '=', False), ('marketplace_seller_id', '!=', False), ('status', '=', 'pending')])
                rec.count_product_pending = len(obj)
            elif rec.state == 'project':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('project', '=', True), ('marketplace_seller_id.user_ids', '=', rec._uid), ('status', '=', 'pending')])
                else:
                    obj = self.env['product.template'].search(
                        [('project', '=', True), ('marketplace_seller_id', '!=', False), ('status', '=', 'pending')])
                rec.count_product_pending = len(obj)
            elif rec.state == 'seller':
                obj = self.env['res.partner'].search(
    ...
    def _get_rejected_count(self):
        for rec in self:
            if rec.state == 'product':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('project', '=', False), ('marketplace_seller_id.user_ids', '=', rec._uid), ('status', '=', 'rejected')])
                else:
                    obj = self.env['product.template'].search(
                        [('project', '=', False), ('marketplace_seller_id', '!=', False), ('status', '=', 'rejected')])
                rec.count_product_rejected = len(obj)
            elif rec.state == 'project':
                if rec.is_user_seller():
                    obj = self.env['product.template'].search(
                        [('project', '=', True), ('marketplace_seller_id.user_ids', '=', rec._uid), ('status', '=', 'rejected')])
                else:
                    obj = self.env['product.template'].search(
                        [('project', '=', True), ('marketplace_seller_id', '!=', False), ('status', '=', 'rejected')])
                rec.count_product_rejected = len(obj)
            elif rec.state == 'seller':
                obj = self.env['res.partner'].search(
    ...
    state = fields.Selection(
        [('product', 'Product'), ('project', 'Project'), ('seller', 'Seller'), ('order', 'Order'), ('payment', 'Payment')])
