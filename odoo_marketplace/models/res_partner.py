...
class ResPartner(models.Model):
    ...
    def _get_default_category_id(self):
        if self._context.get("pass_default_categ"):
            return False
        elif not self.product_seller or not self.project_seller:
            return False
        else:
            if self.product_seller:
                return self.env[product.public.category]\
                    .search([('project', '!=', True)], limit=1)
            else:
                return self.env[product.public.category]\
                    .search([('project', '=', True)], limit=1)
    ...
    state = fields.Selection([
        ...
        ('approved', 'Approved'), 
        ('denied', 'Denied')], string="Seller Status", default="new", copy=False, translate=True, tracking=True)
    ...
    commission = fields.Float(string="Default Sale Commission", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'mp_commission'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False, tracking=True)
    ...
    seller_payment_limit = fields.Integer(string="Seller Payment Limit", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'mp_seller_payment_limit'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False, tracking=True)
    next_payment_request = fields.Integer(string="Next Payment Request", default=lambda self: self.env['ir.default'].get(
        'res.config.settings', 'mp_next_payment_request'), read=['odoo_marketplace.marketplace_draft_seller_group'], write=['odoo_marketplace.marketplace_manager_group'], copy=False, tracking=True)
    ...
    # custom
    product_seller = fields.Boolean(string='Product Seller')
    project_seller = fields.Boolean(string='Project Provider')
    siup_file = fields.Binary(string='SIUP File', attachment=True)
    siup = fields.Char('Bussiness License (SIUP)')
    bussiness_fields = fields.Char('Bussiness Fields')
    contact_name = fields.Char('Contact Person')
    phone_with_code = fields.Char('Phone ', compute='_phone_with_code', store=True, readonly=True)
    project_type_id = fields.Many2one('product.public.category',
        'Project Type', help="Select Project Type")
    ...
    @api.depends('phone', 'country_id.phone_code')    
    def _phone_with_code(self):
        for partner in self:
            phone = partner.phone or ''
            phone_code = partner.country_id.phone_code or ''
            partner.phone_with_code = '+' + str(phone_code) + ' ' + phone if phone_code else phone
    ###

    #pdf file check
    @api.constrains('siup_file')
    def _check_file(self):
       if self.siup and str(self.siup.split(".")[1]) != 'pdf':
            raise ValidationError(
                "Please upload pdf type of file")
    ...
    def change_seller_group_and_send_mail(self):
        ...
        if self.seller and self.state == "approved":
            ...
            # custom
            # originally self.env["product.template"].disable_seller_all_products(self.id)
            ###
