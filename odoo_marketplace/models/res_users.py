...
class ResUsers(models.Model):
    ...
    def _signup_create_user(self, values):
        ...
        print(res)
        if res.partner_id.product_seller:
            product_seller_group_obj = self.env.ref('odoo_marketplace.marketplace_product_seller_group')
            product_seller_group_obj.sudo().write({"users": [(4, res.id, 0)]})
        elif res.partner_id.project_seller:
            project_seller_group_obj = self.env.ref('odoo_marketplace.marketplace_project_seller_group')
            project_seller_group_obj.sudo().write({"users": [(4, res.id, 0)]})
        return res
