...
class Website(models.Model):
    ...
    mp_sale_count = fields.Boolean(
        string="Show seller's sales count on website.")
    mp_shipping_address = fields.Boolean(
        string="Show seller's shipping address on website.")
    mp_seller_since = fields.Boolean(
        string="Show seller since Date on website.")
    ...
    mp_show_become_a_seller = fields.Boolean(
        "Show Become a Seller button on Account Home Page")
    mp_term_and_condition = fields.Html(
    ...
    mp_show_sell_menu_header = fields.Boolean(
        string="Show Sell menu in header")
    mp_show_sell_menu_footer = fields.Boolean(
        string="Show Sell menu in footer")
    mp_marketplace_t_c = fields.Boolean(
        string="Show Marketplace Terms & Conditions on website.")
    mp_inquiry_subject = fields.Char(
        string='Default Inquiry Subject', default='Inquiry request')
    mp_inquiry_message = fields.Text(
        string='Default Inquiry Message', default='Inquiry request details')

    # Website: /shop/product & /shop/invest
    mp_product_category_id = fields.Many2one(
        'product.public.category', string='Product Category')
    mp_invest_category_id = fields.Many2one(
        'product.public.category', string='Product Category ')
    ...
    def validate_mp_config_data(self, vals):
        ...
        if recently_product != None and (recently_product < 1 or recently_product > 20):
            raise UserError(
                _("Recently Added Products count should be in range 1 to 20."))
        review_load_no = vals.get('mp_review_load_no')
        if review_load_no != None and review_load_no < 1:
            raise UserError(
                _("Display Seller Reviews count should be more than 0."))
...
class Menu(models.Model):
    ...
    def _compute_visible(self):
        ...
        for menu in self:
            if '/?' in request.httprequest.full_path or \
                '/#' in request.httprequest.full_path or \
