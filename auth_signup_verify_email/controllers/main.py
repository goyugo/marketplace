...
class SignupVerifyEmail(AuthSignupHome):
    ...
    def passwordless_signup(self):
        ...
        # Remove password
        values["password"] = ""
        sudo_users = request.env["res.users"].with_context(
        ...
        try:
            with request.cr.savepoint():
                ...
                if qcontext.get('is_seller'):
                    values = {key: qcontext.get(key) for key in ('login', 'name', 'is_seller', 'country_id', 'url_handler')}
                    values['seller'] = True
                    seller_type = qcontext.get('seller_type')
                    if seller_type == 'product_seller':
                        values['product_seller'] = True
                    elif seller_type == 'project_seller':
                        values['project_seller'] = True
                else:
